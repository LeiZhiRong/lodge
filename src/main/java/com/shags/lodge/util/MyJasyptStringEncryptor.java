...
public class MyJasyptStringEncryptor implements StringEncryptor {
    ...
    private PBEConfig getConfig(Environment se) {
        SimpleStringPBEConfig config = new SimpleStringPBEConfig();
        config.setPassword(getRequiredProperty(se));
    ...
    private static String getProperty(Environment environment, String key, String defaultValue) {
        ...
        return Objects.isNull(property) ? environment.getProperty(key, defaultValue) : property;
    }


    private static boolean propertyExists(Environment environment, String key) {
        return environment.getProperty(key) == null && environment.getProperty(key) == null;
    }

    private static String getRequiredProperty(Environment environment) {
        if (propertyExists(environment, "jasypt.encryptor.password")) {
            throw new IllegalStateException(String.format("Required Encryption configuration property missing: %s", "jasypt.encryptor.password"));
        }
        String property = environment.getProperty("jasypt.encryptor.password");
        return Objects.isNull(property) ? environment.getProperty("jasypt.encryptor.password") : property;
    }
}
